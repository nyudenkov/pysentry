...
- Per-group fail thresholds are **strictest-wins per context** (`effective_threshold` in `src/audit/pipeline.rs`): a finding's threshold is the MINIMUM across the contexts that reach it - each reaching `[groups.*]` contributes its threshold, and the main/prod context contributes the global `fail_on` ONLY when the package is main-reachable (`build_main_reachable`). Consequence: a group-only package may loosen BELOW global (closes #151), but a main-reachable package keeps global as a floor a group can only tighten. Do NOT collapse this to a plain `min(global, groups)` - that floors every package at global and silently breaks the loosen-dev case (caught in the v0.5.0 review). Both attribution passes run ONLY when `[groups.*]` is configured
- Partial-fetch is **fail-closed by default** (`[sources].fail_on_partial = true`): when a source fails but ≥1 succeeds, the scan continues but exits 2 UNLESS `--no-fail-on-partial`. Total failure (all sources down) is always a hard error regardless of the knob. The partial marker is printed in EVERY format before the exit, so incompleteness is never silent. Do NOT flip the default to lenient
